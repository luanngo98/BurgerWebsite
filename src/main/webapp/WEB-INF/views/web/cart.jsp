...
<body>
	...
	<div id="info" class="best_burgers_area" style="padding-top: 50px">
		<div  class="container">
			<form  action="dat-hang" method="POST">
					...
					<div class="col-lg-6 checkout-left">
						<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right"
								for="form-field-1">User:</label> <input type='text'
								name='username' required="required" />
						</div>
						<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right"
								for="form-field-1">Address:</label> <input type='text'
								name='address' required="required" />
						</div>
						<div class="form-group">
							<label class="col-sm-3 control-label no-padding-right"
								for="form-field-1">Phone:</label> <input type="number"
						...
						<table id="table"
							...
							<tbody>
								...
								<c:forEach items="${order.items}" var="item">
									<tr>
										<td>${item.burger.id}</td>
										<td>${item.burger.name}</td>

										<td><img width="100px" height="100px"
											src="<c:url value= "/images/${item.burger.thumbnailName}" />"></td>
										<td class="hidden-480">${item.price}</td>
										<td class="hidden-480">${item.quantity}&nbsp;&nbsp;
											<div class="btn-group">
												<a href='<c:url value="/tang-so-luong?burgerId=${item.burger.id}" />' class="btn btn-primary btn-sm btn-xs">
													<i class="fa fa-plus"></i>
												</a>
												<a href="<c:url value="/giam-so-luong?burgerId=${item.burger.id}" />" class="btn btn-primary btn-sm btn-xs">
													<i class="fa fa-minus"></i>
												</a>
											</div>
										</td>
										<td class="hidden-480">${item.price*item.quantity}</td>
										<td><input  type="hidden" name='totalPerItem' value="${item.price*item.quantity}"/></td>
										<td>
											<a href="<c:url value="/xoa?burgerId=${item.burger.id}" />" title="Remove item" class="btn btn-sm">
				...
				<div align="right">
					<%-- <a href="<c:url value="/dat-hang" />" class="boxed-btn3">Order</a> --%>
					<c:if test="${empty order.items}">
						<a href="<c:url value="/trang-chu#menu" />"><b>Click vào đây</b></a> để tiếp tục mua hàng
					</c:if>
					<c:if test="${not empty order.items}">
						<button type="submit" class="boxed-btn3">Order</button>
					</c:if>
				</div>
			</form>
		</div>
